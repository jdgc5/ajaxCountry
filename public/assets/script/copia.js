...
document.getElementById('btDeletePais').onclick = () => {
    peticionBorrarPais(code);
};
...
deletePaisModal.addEventListener('shown.bs.modal', function (event) {
    ...
    function llamadaAjax(data) {
        ...
        .then(data => {
            ...
            if(data.result > 0) {
                successAlert.className = 'alert alert-success + clase';
...
function peticionBorrarPais() {
    // Obtén el código del país almacenado previamente
    const code = window.currentDeleteCode;

    fetch('/ruta-del-controlador/' + code, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': csrf 
    ...
    function solucionAriel(data, condicion) {
        ...
        // divSuccessAlert.innerHTML = 'Pais insertado correctamente';
