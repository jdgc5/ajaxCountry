...
(() => {
    ...
    function peticionDatosDeletePais(data) {
        ...
        .then(ajaxData => {
            ...
            infoDelete.innerHTML ='Â¿EstÃ¡s seguro de que quieres eliminar el PaÃ­s "' + deleteName.value + '" con codigo "' + deleteCode.value + '"?';
        })
        .catch(error => console.error("Error:", error));
    } 
    ...
    function solucionAriel(data, condicion) {
        ...
        divSuccessAlert.innerHTML = 'La operaciÃ³n se realizÃ³ correctamente.';
        ...
        // AÃƒÂ±adimos la fila a la cabecera
        head.appendChild(thtr);
        
        // AÃƒÂ±adimos la cabecera a la tabla
        ...
        for(const pais of data.paises) {
            ...
            showBtn.onclick = (e) => {
              alert('PaÃ­s ' + e.target.dataset.name + ' con codigo ' + e.target.dataset.code);
            }
